...
// Finite Difference Method - Abstarct base class
class FDMBase
{
public:
	FDMBase();
...
protected:
	...
	// Spatial diecretization
	double x_dom; 	 // spatial extene [0.0, x_dom]
	unsigned long J; // # spatial differencing points
	double dx;  	 // Spatial step size (calculated from above)
	std::vector<double> x_values;  // stores the coordinates of x dimension

	// Time diesretization
	double t_dom;    // Temporal extene [0.0, t_dom]
	unsigned long N; // # temporal differencing points
	...
	// specific FDM techniques, such as explicit Euler, 
	// Crank-Nichosen, etc.
