...
int main() {
    try {
        ...
        double t2 = double(clock()) / CLOCK_PER_SEC;

        // call kernel
        mc_call(d_s.getData(), T, K, B, S0, sigma, mu, r, dt,
                d_normals.getData(), N_STEPS, N_PATHS);
        cudaDeviceSynchronize();

        double t3 = double(clock()) / CLOCK_PER_SEC;
        ...
        temp_sum /= N_PATHS;  // expected price
        double t4 = double(clock()) / CLOCK_PER_SEC;
        ...
        sum /= N_PATHS;
        double t5 = double(clock()) / CLOCK_PER_SEC;
